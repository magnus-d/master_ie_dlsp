...
def external_chart():
    ...
    ax1.plot(df_ext_data.index, df_ext_data["Ax"], '.-', label="Accel_Ax", linewidth=0.5, ms=1)
    ax1.plot(df_ext_data.index, df_ext_data["Ay"], '.-', label="Accel_Ay", linewidth=0.5, ms=1)
    ax1.plot(df_ext_data.index, df_ext_data["Az"], '.-', label="Accel_Az", linewidth=0.5, ms=1)

    ax2.plot(x_freq, freq_ax, '.-', label="ax_freq", linewidth=0.5, ms=1)
    ax2.plot(y_freq, freq_ay, '.-', label="ay_freq", linewidth=0.5, ms=1)
    ax2.plot(z_freq, freq_az, '.-', label="az_freq", linewidth=0.5, ms=1)
...
class Window(QMainWindow, gui.Ui_MainWindow):
    ...
    def LED_states(self, led_state):
        ...
        self.LED_an_Zustand_4.hide()
        self.LED_an_Zustand_5.hide()
        self.LED_an_Zustand_11.hide()
        self.LED_an_Zustand_21.hide()
        self.LED_an_Zustand_31.hide()
        self.LED_an_Zustand_41.hide()
        self.LED_an_Zustand_51.hide()
        if led_state == 1:
        ...
        if led_state == 4:
            self.LED_an_Zustand_4.show()
            self.LED_an_Zustand_41.show()
        if led_state == 5:
            self.LED_an_Zustand_5.show()
            self.LED_an_Zustand_51.show()
    ...
    def machine_learning_training(self):
        ...
        split_factor = 0.99
        X = df_train_data.iloc[:, 1:102].values
        y = df_train_data.iloc[:, 102].values
        ...
        classifier = MLPClassifier(solver="adam", alpha=0.0001, max_iter=5000, early_stopping=False)
    ...
    def machine_learning_classification(self, sc, classifier, df_new_data):
        freq_data_1, x_time, x_freq = gen_test_data(df_new_data[len(df_new_data)-202:])
        freq_data_2, x_time, x_freq = gen_test_data(df_new_data[len(df_new_data)-404:len(df_new_data)-202])
